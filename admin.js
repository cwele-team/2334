    ...
    fetch(`admin_movies.php?action=get_movie&id=${movieId}`)
        ...
        .then(data => {
            if (data.success) {
                ...
                document.getElementById('editMovieRating').value = movie.ocena_sr;
        ...
        .then(data => {
            if (data.success) {
                ...
                data.categories.forEach(category => {
                    html += `<option value="${category.id}">${category.kategoria}</option>`;
                });
...
function loadLicenses() {
    fetch('admin_categories.php?action=get_licenses')
        ...
        .then(data => {
            if (data.success) {
                const select = document.getElementById('movieLicense');
...
function addCategory() {
    ...
    .then(data => {
        if (data.success) {
            ...
            loadCategoriesList();
            loadCategories(); // Refresh select options
        } else {
