...
function validateMovieData($data, $isUpdate = false) {
    ...
    if (isset($data['ocena_sr']) && !empty($data['ocena_sr'])) {
        if (!is_numeric($data['ocena_sr']) || $data['ocena_sr'] < 0 || $data['ocena_sr'] > 10) {
            $errors[] = 'Ocena musi być liczbą od 0 do 10';
        }
    }

    if (isset($data['rok_produkcji']) && !empty($data['rok_produkcji'])) {
        if (!is_numeric($data['rok_produkcji']) || $data['rok_produkcji'] < 1900 || $data['rok_produkcji'] > 2030) {
            $errors[] = 'Rok produkcji musi być liczbą między 1900 a 2030';
        }
    }
...
switch ($action) {
    ...
    case 'add_movie':
        ...
        $stmt = $conn->prepare("INSERT INTO Filmy (tytul, opis, ocena_sr, czas_trwania, rok_produkcji, kategoria, miniaturka_url, sciezka_do_napisow, dostepne_jakosci, napisy, id_uzytkownika, typ_licencji) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");

        $stmt->bind_param("ssdssisssiii",
            $data['tytul'],
            $data['opis'],
            $data['ocena_sr'] ?? null,
    ...
    case 'update_movie':
        ...
        $allowedFields = ['tytul', 'opis', 'ocena_sr', 'czas_trwania', 'rok_produkcji', 'kategoria', 'miniaturka_url', 'sciezka_do_napisow', 'dostepne_jakosci', 'napisy', 'typ_licencji'];
        ...
        foreach ($allowedFields as $field) {
            if (isset($data[$field]) && $data[$field] !== '') {
                ...
                if (in_array($field, ['ocena_sr'])) {
                    $types .= 'd';
                } elseif (in_array($field, ['rok_produkcji', 'kategoria', 'typ_licencji'])) {
                    $types .= 'i';
                } else {
                    $types .= 's';
                }
